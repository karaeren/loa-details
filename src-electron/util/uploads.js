...
export const upload = async (log) => {
  try {
    ...
    const apiUrl = settings.get("settings.uploads.api");
    const uploadEndpoint = settings.get("settings.uploads.uploadEndpoint");
    const uploadKey = settings.get("settings.uploads.uploadKey");
    const includeRegion = settings.get("settings.uploads.includeRegion");

    if (includeRegion) session.region = settings.get("settings.general.server");
    session.unlisted = settings.get("settings.uploads.uploadUnlisted");
